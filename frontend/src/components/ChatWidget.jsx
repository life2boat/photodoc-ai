...
export default function ChatWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen && (
        <button onClick={() => setIsOpen(true)} aria-label="Открыть чат" className="bg-yellow-400 hover:bg-yellow-500 text-black p-4 rounded-full shadow-2xl transition-transform hover:scale-105">
      ...
      {isOpen && (
        <div className="flex flex-col w-80 sm:w-96 h-[500px] bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl overflow-hidden">
          <div className="bg-gray-950 p-4 flex justify-between items-center border-b border-gray-800">
            ...
            <button onClick={() => setIsOpen(false)} aria-label="Закрыть чат" className="text-gray-400 hover:text-white">✕</button>
          </div>

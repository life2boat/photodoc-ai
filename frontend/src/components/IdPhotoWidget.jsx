...
const PHOTO_FORMATS = [
  ...
  { id: '3.5x4.5', label: '3.5х4.5 см (Паспорт РФ, Загранпаспорт, Визы)', price: 300 },
  { id: '4x6', label: '4х6 см', price: 300 },
  { id: '9x12', label: '9х12 см (Личное дело)', price: 300 },
];
...
export default function IdPhotoWidget({ onSuccess, onReset }) {
  ...
  const handleSubmitOrder = async () => {
    ...
    const formatObj = PHOTO_FORMATS.find(f => f.id === selectedFormat);
    const formatLabel = formatObj ? formatObj.label : selectedFormat;
  ...
  if (isSuccess) {
    return (
      <div className="bg-green-50 border border-green-200 text-green-800 rounded-xl p-8 text-center space-y-4 animate-in fade-in zoom-in duration-300 max-w-2xl mx-auto">
        ...
        <p className="text-green-700">Мы подготовим фото по стандартам и свяжемся с вами.</p>
        ...
        <div className="space-y-6 animate-in fade-in duration-300 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              <select value={selectedFormat} onChange={(e) => setSelectedFormat(e.target.value)} disabled={isLoading} className="w-full border border-gray-700 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-yellow-400 bg-gray-800 text-white">
                {PHOTO_FORMATS.map(format => (<option key={format.id} value={format.id}>{format.label} — {format.price} ₽</option>))}
              </select>
            </div>
            <div>
